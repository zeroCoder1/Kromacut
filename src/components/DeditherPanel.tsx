...
export const DeditherPanel: React.FC<Props> = ({ canvasRef, onApplyResult }) => {
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
        ...
        for (let x = 0; x < w; x++) {
          ...
          for (const [dx, dy] of neigh) {
            ...
            if (nr === r && ng === g && nb === b && na === a) {
              ...
              const key =
                ((nr & 0xff) << 24) | ((ng & 0xff) << 16) | ((nb & 0xff) << 8) | (na & 0xff);
      ...
      const outBlob = await new Promise<Blob | null>((res) => c.toBlob((b) => res(b), 'image/png'));
      if (!outBlob) return;
  ...
  return (
    <div className="controls-group">
      ...
      <div>
        <button className="apply-btn" onClick={handleApply} disabled={working}>
