...
export default function ThreeDControls({ swatches, onChange, persisted }: ThreeDControlsProps) {
  ...
  const [baseSliceHeight, setBaseSliceHeight] = useState<number>(persisted?.baseSliceHeight ?? 0.2);
  ...
  const prevOrderRef = useRef<number[]>(persisted?.colorOrder ? persisted.colorOrder.slice() : []);
  ...
  const handleDrop = useCallback(
    (e: DragEvent<HTMLDivElement>, toDisplayIdx: number) => {
      ...
      const currentOrder =
        colorOrder.length === filtered.length ? colorOrder.slice() : filtered.map((_, i) => i);
  ...
  useEffect(() => {
    ...
    onChange(next);
  }, [onChange, layerHeight, baseSliceHeight, colorSliceHeights, colorOrder, filtered, pixelSize]);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {
            ...
            (colorOrder.length === filtered.length ? colorOrder : filtered.map((_, i) => i)).map(
              (fi, displayIdx) => {
                const s = filtered[fi];
